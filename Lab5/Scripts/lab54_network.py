...
def run():
    ...
    """
        ...
        bw = None,
 		...
 		max_queue_size = None 
    """
    link_h1sw1.intf1.config( bw=2, enable_ecn=True,enable_red=True)
    link_h2sw1.intf1.config( bw=20, enable_ecn=True, enable_red=True)
    link_r1sw2.intf1.config( bw=2.5, enable_ecn=True, enable_red=True)
    link_r2h4.intf1.config( bw=20, enable_ecn=True, enable_red=True)
    link_r2h3.intf1.config( bw=0.5, enable_ecn=True, enable_red=True)

    net.start()
    r1.cmd('ifconfig r1-eth0 10.0.0.10 netmask 255.255.255.0')
    r1.cmd('ifconfig r1-eth1 10.0.1.10 netmask 255.255.255.0')
    r2.cmd('ifconfig r2-eth2 10.0.2.20 netmask 255.255.255.0')
    r1.cmd('echo 1 > /proc/sys/net/ipv4/ip_forward')
    r2.cmd('ifconfig r2-eth1 10.0.3.20 netmask 255.255.255.0')
    r2.cmd('ifconfig r2-eth0 10.0.1.20 netmask 255.255.255.0')
